...
#'
#' # ts class - default
#' air_passengers <- as_torch_tensor(AirPassengers)
...
#' air_passengers <-
#'  as_torch_tensor(AirPassengers, 6)
#'
#' class(air_passengers)
#' dim(air_passengers)
#'
#' @export
as_torch_tensor <- function(data, ..., dtype = NULL,
                      device = NULL,
                      requires_grad = FALSE,
                      pin_memory = FALSE){
  # TODO: rename to torch_tensor
  UseMethod("as_torch_tensor")
}

#' @export
#' @rdname as_torch_tensor
as_torch_tensor.default <- function(data, dtype = NULL,
                              device = NULL,
                              requires_grad = FALSE,
                              pin_memory = FALSE){
  torch::torch_tensor(
...
#' @export
#' @rdname as_torch_tensor
as_torch_tensor.torch_tensor <- function(data, ...,
                                   dtype = NULL,
                                   device = NULL,
                                   requires_grad = FALSE){
  data <- data$to(
...
#' @export
#' @rdname as_torch_tensor
as_torch_tensor.data.frame <- function(data, ...,
                      dtype = NULL,
                      device = NULL,
                      requires_grad = FALSE,
                      pin_memory = FALSE){
  ...
  # case as_torch_tensor(euro_stock, name)
