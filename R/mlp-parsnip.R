...
#'
#' @details
#' This is a `parsnip` API to the lagged feed-forward networks (aka MLP). For now the only
...
#'
#' @export
lagged_mlp <- function(mode = "regression",
                timesteps = NULL,
                horizon = 1,
                learn_rate = 0.01,
                epochs = 50,
...
make_lagged_mlp <- function(){
  ...
  parsnip::set_dependency("lagged_mlp", "torchts", "torch")
  parsnip::set_dependency("lagged_mlp", "torchts", "torchts")
